...
import {criteria_4_4_1} from "../../models/criteria_4_model/Criteria_4_Model.js";
...
const router = express.Router();
  ...
  router.get("/", async (request, response) => {
    try {
      const tables = await criteria_4_4_1.find({});
  ...
  router.put("/:id", async (request, response) => {
    try {
      ...
      const updatedRow = await criteria_4_4_1.findByIdAndUpdate(
        { _id: id },
